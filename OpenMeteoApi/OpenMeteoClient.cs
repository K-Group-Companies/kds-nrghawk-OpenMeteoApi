...
public class OpenMeteoClient
{
		...
		params KeyValuePair<string, string>[] parameters)
	{
		...
		if (!result.IsSuccessStatusCode)
			throw new Exception($"Failed to get weather data. Status Code: {result.StatusCode}.");
		...
		params KeyValuePair<string, string>[] parameters)
	{
		...
		var result = await _httpClient.SendAsync(new HttpRequestMessage(HttpMethod.Get, new Uri(urlString)));
		if (!result.IsSuccessStatusCode)
			throw new Exception($"Failed to get air quality data. Status Code: {result.StatusCode}.");
		var str = await result.Content.ReadAsStringAsync();
	...
	public async Task<List<Geolocation>> GetLocationsByName(string name, int? count = null, params KeyValuePair<string, string>[] parameters)
	{
		...
		var result = await _httpClient.SendAsync(new HttpRequestMessage(HttpMethod.Get, new Uri(urlString)));
		if (!result.IsSuccessStatusCode)
			throw new Exception($"Failed to get geolocation data. Status Code: {result.StatusCode}.");
		var str = await result.Content.ReadAsStringAsync();
		...
		params KeyValuePair<string, string>[] parameters)
	{
		...
		var result = await _httpClient.SendAsync(new HttpRequestMessage(HttpMethod.Get, new Uri(urlString)));
		if (!result.IsSuccessStatusCode)
			throw new Exception($"Failed to get historical weather data. Status Code: {result.StatusCode}.");
		var str = await result.Content.ReadAsStringAsync();
		return (JsonSerializer.Deserialize(str, typeof(WeatherData), SourceGenerationContext.Default) as WeatherData)!;
	}

	#endregion HistoricalWeather
}